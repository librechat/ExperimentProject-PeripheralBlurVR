...
Draw = function(conditionIndex=0, participantIndex=0, sessionIndex = 0,
                printLines=TRUE, printAverage=TRUE, groupWithSession=FALSE,gradientByAnotherFactor=FALSE){
...
DrawAverage = function(groupWithSession=FALSE,gradientByAnotherFactor=FALSE,printArrows=FALSE){
  ...
  lapply(group, function(groupIndex){
    ...
    discomfortSumup = lapply(timeStampIndex, function(t){
      sum = 0
      valueAtTimeStamp = sapply(seq_along(extrudedDiscomfortRecord), function(index){
        if(groupWithSession && session[index] != groupIndex) return(0)
        if(!groupWithSession && condition[index] != groupIndex) return(0)
        
        return (extrudedDiscomfortRecord[[index]]$Discomfort[t])
      })
      
      avg = mean(valueAtTimeStamp[valueAtTimeStamp!=0])
      sd = sd(valueAtTimeStamp[valueAtTimeStamp!=0])
...
discomfortTotal = do.call(rbind,extrudedDiscomfortRecord)
